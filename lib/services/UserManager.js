/**
 * Enhanced User Management System for STEP Timer
...
const hashPassword = async (password, salt = null, iterations = CRYPTO_CONFIG.pbkdf2Iterations) => {
  ...
  try {
    ...
    const saltToUse = salt || crypto.getRandomValues(new Uint8Array(CRYPTO_CONFIG.saltSize));
    ...
    const fallbackSalt = salt || crypto.getRandomValues(new Uint8Array(CRYPTO_CONFIG.saltSize));
...
export const validateUsername = (username) => {
  ...
  if (trimmedUsername.length > USERNAME_CONFIG.maxLength) {
    return {
      isValid: false,
      error: AUTH_MESSAGES.usernameTooLong
    };
  }
...
export class UserManager {
  constructor() {
    this.usersKey = STORAGE_KEYS.registeredUsers;
    this.sessionsKey = STORAGE_KEYS.userSessions;
    this.sessionTimeout = SESSION_CONFIG.timeout;
    this.extendedSessionTimeout = SESSION_CONFIG.extendedTimeout;
    this.maxLoginAttempts = SECURITY_CONFIG.maxLoginAttempts;
    this.accountLockTime = SECURITY_CONFIG.accountLockTime;
  ...
  async registerUser(userId, userData = {}) {
    // Enhanced input validation - v2
    if (!userId || typeof userId !== 'string') {
      throw new Error('유효한 사용자 ID가 필요합니다.');
    }

    // Validate username format only (uniqueness checked by API)
    ...
    // Validate email format only (uniqueness checked by API)
    const emailValidation = validateEmail(userData.email);
    if (!emailValidation.isValid) {
      throw new Error(emailValidation.error);
    }

    // Use Supabase API for registration (API handles uniqueness check)
    try {
  ...
  initializeUserStats(userId) {
    ...
    const userSessionsKey = `stepSessions_${userId}`;
  ...
  getActiveSession(userId) {
    const activeSessionKey = `activeSTEPSession_${userId}`;
    const sessionData = safeLocalStorage.getItem(activeSessionKey);
    return sessionData ? JSON.parse(sessionData) : null;
  }

  createSTEPSession(userId, sessionData) {
    const now = new Date();
    let startTime;
    ...
    const activeSession = {
      id: sessionId,
      title: sessionData.title || POMODORO_CONFIG.defaultSessionTitle,
      goal: sessionData.goal || '',
      tags: sessionData.tags || '',
      location: sessionData.location || '',
      duration: sessionData.duration || POMODORO_CONFIG.defaultDuration,
  ...
  completeSTEPSession(userId, sessionId) {
    const activeSessionKey = `activeSTEPSession_${userId}`;
  ...
  stopSTEPSession(userId, sessionId) {
    const activeSessionKey = `activeSTEPSession_${userId}`;
    safeLocalStorage.removeItem(activeSessionKey);
  ...
  deleteUser(userId) {
    ...
    const keysToDelete = [
      `userStats_${userId}`,
      `stepSessions_${userId}`,
      `activeSTEPSession_${userId}`,
      `meetings_${userId}`
    ];
  ...
  getUpcomingMeetings(userId, limit = PAGINATION_CONFIG.upcomingMeetingsLimit) {
    const meetings = this.getMeetings(userId);
    const now = new Date();
