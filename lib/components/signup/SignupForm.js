...
export function SignupForm({ mode, onModeChange }) {
  ...
  return (
    <Form onSubmit={handleSubmit}>
      ...
      <InputGroup>
        <InputLabel>로그인 ID</InputLabel>
        <InputField
          type="text"
          value={formData.username}
          onChange={(event) => handleChange('username', event.target.value)}
          placeholder="로그인 ID 입력 (4-20자, 영문/숫자)"
          $error={errors.username}
          autoComplete="username"
        />
        <HelperText>로그인 시 사용할 고유 ID입니다</HelperText>
    {isChecking.username && !errors.username && <HelperText>로그인 ID 확인 중...</HelperText>}
