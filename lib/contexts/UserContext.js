...
export const UserProvider = ({ children }) => {
  ...
  const loginUser = async (userId, password, rememberMe = false) => {
    try {
      ...
      // Input validation
      if (!userId?.trim()) {
        throw new Error(AUTH_MESSAGES.usernameRequired);
      }

      if (!password) {
        throw new Error(AUTH_MESSAGES.passwordRequired);
      }
      ...
      if (!response.ok) {
        throw new Error(result.error || AUTH_MESSAGES.loginFailed);
      }
  ...
  const registerUser = async (userId, userData) => {
    try {
      ...
      if (!userData?.password || userData.password.length < PASSWORD_CONFIG.minLength) {
        throw new Error(AUTH_MESSAGES.passwordMinLength);
      }
