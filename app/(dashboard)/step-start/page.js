...
const STEPInputSection = styled.div`
  background: var(--background-primary);
...
export default function STEPStartPage() {
  ...
  const { createSTEPSession, currentUser } = useUser();
  ...
  const handleStart = async () => {
    try {
      ...
      // Create STEP session
      const sessionData = {
        title: formData.goal || 'STEP 세션',
        goal: formData.goal || '',
        duration: formData.duration,
        scheduledTime: combinedDateTime.toISOString()
      };

      console.log('Creating STEP session with data:', sessionData);
      const result = await createSTEPSession(sessionData);
      console.log('STEP session created successfully:', result);

      // Navigate back to main page
      router.push('/main');

    } catch (error) {
      console.error('Failed to start STEP session:', error);
      console.error('Error details:', error.message, error.stack);
      alert(`STEP 세션 시작에 실패했습니다.\n에러: ${error.message || '알 수 없는 오류'}`);
    }
  };
  ...
  return (
    <StartContainer>
      <AppHeader>
        ...
        <AppTitle>STEP 시작하기</AppTitle>
        <HeaderButton onClick={handleStart}>시작</HeaderButton>
      </AppHeader>

      <MainContent>
        <STEPInputSection>
          <SectionLabel>STEP 설정</SectionLabel>
