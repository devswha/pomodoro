...
const getAchievementText = (score) => {
  if (score >= 2000) return 'STEP 마스터';
  if (score >= 1500) return 'STEP 전문가';
  if (score >= 1000) return 'STEP 숙련자';
  if (score >= 500) return 'STEP 입문자';
  return 'STEP 새싹';
};
          ...
          {rankingData.length > 0 ? (
            ...
            <NoDataMessage>
              ...
              <EmptyRankingDescription>
                STEP 세션을 완료한 사용자가 없습니다.<br />
                첫 번째 STEP를 시작해서 랭킹에 도전해보세요!
              </EmptyRankingDescription>
              <EmptyRankingButton onClick={() => router.push('/step-start')}>
                STEP 시작하기
              </EmptyRankingButton>
            </NoDataMessage>
          )}
